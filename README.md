...
- **商品详情与图片缓存**：详情页展示商品关键指标、趋势和主图；商品图按白名单域名联网缓存，后续可本地复用。
- **关键词追踪冷启动**：全新关键词即使尚无商品池，也可生成首轮低频采集任务，逐步积累历史快照。
- **研究项目与决策报告**：把候选商品、对标商品、核心关键词、市场利基和人工记录组织成项目，并生成可复核的中文决策报告。
- **项目复核与人工观察计划**：按证据时效、趋势跨度、追踪状态和人工节奏管理项目回看，不把计划误做成无人值守采集。
- **详情证据工作区**：围绕项目成员下钻详情缺口，区分首次采集、刷新过期、本地回填、页面未提供和人工核对等处置类型。
- **领域评分模型实验室**：支持版本化领域参数、单品试算和批量对照，用于影子验证评分策略，不直接替换生产榜单。
